-- Story Campaigns Database Schema for Phase 2 Integration
...
-- Story Choices: Available narrative decisions
...
-- Player Choices: Track player decisions
...
CREATE INDEX idx_story_choices_episode_id ON story_choices(episode_id);
CREATE INDEX idx_story_choices_choice_key ON story_choices(choice_key);
CREATE INDEX idx_player_choices_player_id ON player_choices(player_id);
CREATE INDEX idx_player_choices_episode_id ON player_choices(episode_id);
...
CREATE POLICY "Story choices are viewable by authenticated users" ON story_choices
    FOR SELECT USING (auth.role() = 'authenticated');

-- Player-specific data policies
CREATE POLICY "Users can view their own choices" ON player_choices
    FOR SELECT USING (auth.uid() = player_id);

CREATE POLICY "Users can insert their own choices" ON player_choices
...
INSERT INTO story_choices (episode_id, choice_key, title, description, choice_text, consequences, next_choice_key, choice_order) 
SELECT 
    e.id,
    'ep1_start',
    'First Contact',
